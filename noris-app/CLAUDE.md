...
- **App Router structure**: All routes live under `app/[locale]/(site)/` (a dynamic locale segment wrapping a route group), e.g. `app/[locale]/(site)/auth/signin`, `app/[locale]/(site)/blog`, `app/[locale]/(site)/docs`, `app/[locale]/(site)/support`. `app/[locale]/(site)/layout.tsx` sets up fonts/metadata, validates the locale, and wraps children in `Provider.tsx`.
- **`app/[locale]/(site)/Provider.tsx`** is a client component (`"use client"`) that wires up global chrome for every page: `ThemeProvider` (next-themes, light/dark mode), `Lines`, `Header`, `ToasterContext` (react-hot-toast), `Footer`, and `ScrollToTop`.
- **i18n**: `next-intl` provides English/French/German translations via locale-prefixed routes (`/en`, `/fr`, `/de`). `middleware.ts` handles locale detection/routing, `i18n/routing.ts` defines the supported locales, `i18n/request.ts` loads messages from `messages/{locale}.json`, and `i18n/navigation.ts` exports locale-aware `Link`/`useRouter`/etc. Message files currently exist but are empty placeholders.
